...
function formatFileName(capabilities, formatString, tag) {
    const defaults = {
        ...
        platformVersion:
            capabilities.platformVersion || 'no-platform-version-in-caps',
        appearance: capabilities.appearance || 'no-appearance-in-caps',
        tag,
    }
...
function getAndCreatePath(capabilities, folder, savePerDevice) {
    ...
    const deviceFolder = deviceName
        ? deviceName.replace(/ /g, '_').toLowerCase()
        : NO_DEVICE_NAME

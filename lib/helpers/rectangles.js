import { NAVIGATION_BAR_SELECTORS, STATUS_BAR_SELECTORS } from './constants';
...
export async function determineIgnoreRectangles(bufferedScreenshot, options) {
    ...
    if (browser.isAndroid && (options.blockOutStatusBar || options.elementBlockOuts.length > 0)) {
...
async function determineStatusBarRectangles(dpr, width) {
    ...
    try {
        statusBarSize = await browser.getElementSize(STATUS_BAR_SELECTORS[browser.isIOS ? 'IOS' : 'ANDROID'])
    } catch (e) {
        console.log(yellow('\nWARNING: The statusbar could not be determined and has been set to the defaults!\n'));
        statusBarSize = { height: 26, width };
    }
...
async function determineNavigationBarRectangles(dpr, width) {
    ...
    let navigationBarSize;
    let navigationBarPosition;

    try {
        navigationBarSize = await browser.getElementSize(NAVIGATION_BAR_SELECTORS.ANDROID);
        navigationBarPosition = await browser.getLocation(NAVIGATION_BAR_SELECTORS.ANDROID);
    } catch (e) {
        console.log(yellow('\nWARNING: The navigation bar could not be determined and has been set to the defaults!\n'));
        navigationBarSize = { height: 0, width: 0 };
        navigationBarPosition = { x: 0, y: 0 };
    }

    NAVIGATION_BAR_DIMENSIONS = multiplyIosObjectValuesAgainstDPR({
        height: navigationBarSize.height,
        width: navigationBarSize.width,
        x: navigationBarPosition.x,
        y: navigationBarPosition.y
    }, dpr);
    ...
    for (let component of components) {
        ...
        try {
            ...
            if (Array.isArray(size)) {
                ...
                dimensions.push(determineDimensions(size, coordinates, margin));
            }
        } catch (e) {
            console.log(yellow(`\nWARNING: The element with selector '${selector}' could not be found!\n`));
        }
    }

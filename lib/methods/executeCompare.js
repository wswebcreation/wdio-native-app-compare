...
export async function executeCompare(folders, fileName, options) {
    ...
    const ignoredBoxes =
        options.output && options.output.ignoredBoxes
            ? [...options.output.ignoredBoxes]
            : []

    // Now adjust the ignored boxes array for the ResembleJS API (which is a total mess :( )
    if (ignoredBoxes.length > 0) {
        options.output.ignoredBoxes = options.output.ignoredBoxes.map(
            (ignoredBox) => {
                return {
    ...
    if (misMatchPercentage > options.saveAboveTolerance || options.debug) {
        ...
        await saveBase64Image(
            ...
            await addBlockOuts(
                Buffer.from(data.getBuffer()).toString('base64'),
                ignoredBoxes
            )
        )
    }

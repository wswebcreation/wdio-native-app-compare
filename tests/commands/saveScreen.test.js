...
describe('saveScreen', () => {
    it('should be able to return the save screen data', async function () {
        const determineFileDataSpy = jest
            .spyOn(Utils, 'determineFileData')
            .mockReturnValue({ fileName: 'fileName.png', filePath: 'filePath' })
        const takeBase64ScreenshotSpy = jest
            .spyOn(Screenshot, 'takeBase64Screenshot')
            .mockReturnValue(IMAGE_STRING)
        const saveBase64ImageSpy = jest
            .spyOn(Save, 'saveBase64Image')
            .mockResolvedValue()

        const data = await saveScreen.call(
            {
                capabilities: {},

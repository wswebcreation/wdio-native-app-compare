...
describe('rectangles', () => {
    ...
    beforeEach(() => {
        ...
        global.driver = {
            ...
            getElementRect: jest
                .fn()
                .mockResolvedValue({ x: 1, y: 2, width: 10, height: 20 }),
            getSettings: jest
                .fn()
                .mockResolvedValue({ allowInvisibleElements: false }),
    ...
    describe('determineIgnoreRectangles', () => {
        it('should return nothing if no ignore rectangles need to be returned', async () => {
            expect(
                await Rectangles.determineIgnoreRectangles(
                    IMAGE_STRING,
                    ignoreRectanglesOptions
                )
            ).toMatchSnapshot()
        ...
        it('should get the block outs', async () => {
            ignoreRectanglesOptions.blockOuts = [
        ...
        it('should return iPhone 12 bottom bar rectangles', async () => {
            getDeviceInfoSpy.mockResolvedValue({
                ...deviceInfo,
                isMediumIphone: true,
            })
            ignoreRectanglesOptions.blockOutIphoneHomeBar = true
        ...
        it('should call the settings API for android when elements needs to be blocked out', async () => {
            global.driver.isAndroid = true
            ignoreRectanglesOptions.elementBlockOuts = [
            ...
            expect(global.driver.updateSettings).toBeCalledWith({
                allowInvisibleElements: true,
            })
            expect(global.driver.updateSettings).toHaveBeenLastCalledWith({
    ...
    describe('determineValidBlockOuts', () => {
        it('should return the valid blockouts if an valid array if provided', () => {
            ...
            expect(
                Rectangles.determineValidBlockOuts(blockOuts)
            ).toMatchSnapshot()
        })
    ...
    describe('determineElementBlockOuts', () => {
        it('should return an empty array if no elements are provided', async () => {
            expect(
                await Rectangles.determineElementBlockOuts({
                    components: [],
                    dpr: 1,
                })
            ).toMatchSnapshot()
        ...
        it('should return an array of one element blockout if 1 element is provided', async () => {
            ...
            const components = [{ element: { selector } }]
            global.$$ = () => [{ 'element-selector-1': {} }]
        ...
        it('should return an array of one element blockout if multiple elements are provided but 1 with an element number', async () => {
            const selector = 'element-selector-1'
            const components = [{ element: { selector }, elementNumber: 1 }]
            global.$$ = () => [
                { 'element-selector-1': {} },
                { 'element-selector-1': {} },
                { 'element-selector-1': {} },
    ...
    describe('determineDimensions', () => {
        it('should be able to determine the dimensions when no margin is provided', () => {
            expect(
                Rectangles.determineDimensions(
                    {
                        x: 100,
                        y: 200,
                        width: 1000,
    ...
    describe('getElementRectangles', () => {
        it('should return element rectangles', async () => {
            expect(
                await Rectangles.getElementRectangles({ elementId: 1 })
            ).toMatchSnapshot()
